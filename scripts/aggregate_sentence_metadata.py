...
argv = sys.argv[1:]
if len(argv) < 4:
	print "usage: aggregate_sentence_metadata.py <amr_file> <sentence_file> <pos_file> <ner_file> <dep_parse_file> <output_dir>"
	sys.exit()
...
if write_to_file:
	with open("dep_parse", 'w') as f:
		for each_id in dep_parse:
			f.write(each_id)
			f.write('\n')
			f.write(str(dep_parse[each_id]))
			f.write('\n')
			f.write('\n')

	
pickle.dump(amr_aggregated_metadata, open(os.path.join(output_dir, "amr_aggregated_metadata.p"), "wb"))
pickle.dump(dep_parse, open(os.path.join(output_dir, "dep_parse.p"), "wb"))
