...
def read_amr(f):
	...
	for each_line in f:
		...
		if each_line.startswith("# ::id"):
			...
			ids_in_order.append(id)
			id_lines_in_order.append(each_line)
		elif each_line.startswith("# ::snt"):
			sent_lines_in_order.append(each_line)
		elif each_line.startswith("# ::save-data") or each_line.startswith("# ::tok"):
			continue
	...
	return amr_dict, ids_in_order, id_lines_in_order, sent_lines_in_order
...
class ConceptRelationLearning(pyvw.SearchTask):
	...
	def predict_relation(self, sentence, k, j, i, prediction, force_edge=False):
		"""
		...
		try:
			examples = example_cache[sentence.id][(k,j)][((par_concept, child_concept), force_edge)]
		except KeyError:
			...
			example_cache[sentence.id][(k,j)][((par_concept, child_concept), force_edge)] = examples
	...
	def _run(self, sentence):
		...
		for nodes in components:
			if root_index in nodes:
				component_roots.append(root_index)
			else:
				for n1 in nodes:
		...
		for k in xrange(n):
			...
			for j in xrange(k):
				if k == root_index:
					#import pdb
					#pdb.set_trace()
					if j in component_roots:
						self.predict_relation(sentence, k, j, i, predictions, force_edge=True)
						if predictions[i] != edgeLabels["NOEDGE"]:
							graph.add_edge(k,j)
				i += 1
			if k == root_index:
